...
type AlipayNotify struct {
	...
	AuthAppID         string  `form:"auth_app_id"`         // 授权方的appid，由于本接口暂不开放第三方应用授权，因此auth_app_id=app_id
	...
	BuyerEmail        string  `form:"buyer_email"`         // 买家支付宝账号，可以是Email或手机号码。
	TotalFee          string  `form:"total_fee"`           // 该笔订单的总金额。请求时对应的参数，原样通知回来。
